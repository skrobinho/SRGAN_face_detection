...
class SRGAN():
    def __init__(self, lr_height=64, lr_width=64, channels=3, upscaling_factor=4, training=True):
        ...
        self.gen_loss = 'mse'
        self.content_loss = 'mse'
        self.adversarial_loss = 'binary_crossentropy'
        self.loss_weights = {"Content" : 1.0, "Adversarial" : 1e-3}
    ...
    #TODO: add saving weights always when losses are lower than previous ones
    ...
    def build_srgan(self):
        ...
        generated_features = self.vgg(sr_img)
        ...
        # Create the model
        model = Model(inputs=[lr_img, hr_img], outputs=[sr_validation, generated_features])
    ...
    def compile_srgan(self, model):
        ...
        model.compile(
            loss=[self.adversarial_loss, self.content_loss],
    ...
    def train(self,
        ...
        :param string test_datapath: test dataset path
        :param int test_interval: interval of generating test images
        :param int batch_size: size of mini batch
        :param strig weights_path: path where network weights are saved
        :param int weights_frequency: frequency of saving weights
        ...
        for epoch in range(epochs):
            ...
            # Train the generator
            gan_loss = self.srgan.train_on_batch([lr_imgs, hr_imgs], [real, imgs_features])

            # Generator and discriminator losses and their means
            loss["G"].append(gan_loss[0])
            loss["D"].append(discriminator_loss[0])

            gan_avg_loss = np.array(loss["G"]).mean(axis=0)
            dis_avg_loss = np.array(loss["D"]).mean(axis=0)

            elapsed_time = datetime.datetime.now() - start_time

            if epoch % print_frequency == 0:
                print(f"{epoch+1}/{epochs} epoch time: {elapsed_time} | GAN loss: {gan_avg_loss:.2f} | Discriminator loss: {dis_avg_loss:.4f}")
                
            #if epoch == (epochs-1) and weights_path:
            if ((epoch % 5000 == 0) or epoch == (epochs-1)) and weights_path:
                self.save_weights(weights_path)

            if (epoch % test_interval == 0) and test_datapath:
                self.test_images(epoch, test_data_loader, path=test_datapath)